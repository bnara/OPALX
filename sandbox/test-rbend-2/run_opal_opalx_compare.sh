...
run_root="${1:-/tmp/opalx-rbend-2-compare}"
...
cp "${script_dir}/test-rbend-2.in" \
   "${script_dir}/test-rbend-2_distribution.txt" \
   "${opalx_dir}/"
cp "${script_dir}/test-rbend-2_opal.in" \
   "${script_dir}/test-rbend-2_opal_distribution.txt" \
   "${opal_dir}/"

(
    cd "${opalx_dir}"
    "${opalx_exe}" test-rbend-2.in
)
