#!/usr/bin/env python3
"""Compare OPALX and old OPAL RBEND output for test-rbend-2.
...
def bend_geometry_markers(*, coordinate: str) -> dict[str, float]:
    ...
    if coordinate == "s":
        nominal_exit = nominal_entry + reference_length
        downstream_support_end = nominal_exit + exit_support
    elif coordinate == "z":
        nominal_exit = nominal_entry + BEND_LENGTH
...
def write_plot(
    ...
    ax.set_ylabel("x [m]")
    ax.set_title("test-rbend-2 design path")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("test-rbend-2 design momentum")
    ax.legend(ncol=2)
    fig.tight_layout()
    ...
    ax.set_title("test-rbend-2 design-path sampled field")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_xlabel("s relative to line start [m]")
    ax.set_ylabel("sampled vertical B field")
    ax.set_title("test-rbend-2 reference-particle sampled field")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("test-rbend-2 vertical particle motion")
    ax.legend()
    fig.tight_layout()
...
def main() -> int:
    ...
    opalx_design = read_design_path(args.opalx_dir / "data/test-rbend-2_DesignPath.dat")
    opal_design = read_design_path(args.opal_dir / "data/test-rbend-2_opal_DesignPath.dat")
    opalx_stat = read_sdds_stat(args.opalx_dir / "test-rbend-2.stat")
    opal_stat = read_sdds_stat(args.opal_dir / "test-rbend-2_opal.stat")
    ...
    summary = "\n".join(
        [
            "# test-rbend-2 OPALX vs OPAL 2022.1",
            "",
