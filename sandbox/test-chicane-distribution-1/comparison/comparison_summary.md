# test-chicane-distribution-1 comparison

Selected monitor step: `Step#1`
Monitor `SPOS`: `9.253200162550e+00` m
Particles at exit: `125`

## Manufactured longitudinal check

- Reference `R56`: `-4.333333333333e-02` m
- Fitted transport `R56`: `-4.327467367520e-02` m
- Difference: `5.865965813612e-05` m
- Fitted monitor offset: `-1.463199339424e-03` m
- Raw `z` residual RMS: `1.463981770955e-03` m
- Raw `z` residual max abs: `1.499248064543e-03` m
- Intercept-corrected `z` residual RMS: `4.785727528281e-05` m
- Intercept-corrected `z` residual max abs: `8.407208624630e-05` m

## Transverse closure diagnostics

- `y` residual RMS: `0.000000000000e+00` m
- `y` residual max abs: `0.000000000000e+00` m
- final `x` mean: `-1.493017430517e-04` m
- final `x` RMS: `3.068619584001e-04` m

## Checks

- `all_particles_at_exit`: `True`
- `r56_within_tolerance`: `True`
- `z_rms_within_tolerance`: `True`
- `z_max_within_tolerance`: `True`
- `y_rms_within_tolerance`: `True`

Overall: `PASS`
