#!/usr/bin/env python3
"""Compare OPALX and old OPAL RBEND output for test-rbend-1.
...
def write_plot(
    ...
    ax.set_ylabel("x [m]")
    ax.set_title("test-rbend-1 design path")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("test-rbend-1 design momentum")
    ax.legend(ncol=2)
    fig.tight_layout()
    ...
    ax.set_title("test-rbend-1 design-path sampled field")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_xlabel("s relative to entrance [m]")
    ax.set_ylabel("sampled vertical B field")
    ax.set_title("test-rbend-1 reference-particle sampled field")
    ax.legend()
    fig.tight_layout()
...
def main() -> int:
    ...
    opalx_design = read_design_path(args.opalx_dir / "data/test-rbend-1_DesignPath.dat")
    opal_design = read_design_path(args.opal_dir / "data/test-rbend-1_opal_DesignPath.dat")
    opalx_stat = read_sdds_stat(args.opalx_dir / "test-rbend-1.stat")
    opal_stat = read_sdds_stat(args.opal_dir / "test-rbend-1_opal.stat")
    ...
    summary = "\n".join(
        [
            "# test-rbend-1 OPALX vs OPAL 2022.1",
            "",
