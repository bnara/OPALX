...
\section{Common analytic RBEND geometry}

The \code{RBEND} tests use the same bend angle and field model as the
\code{SBEND} tests, but the geometry is rectangular.  The input length is the
straight rectangular body length,
\[
  L_\mathrm{rect}=1.0\ \mathrm{m},\qquad
  \theta=45\degree,\qquad
  E_1=0,\qquad E_2=\theta .
\]
The reference trajectory is still compared with a circular arc.  For a
rectangular bend the arc radius and arc length are derived from the straight
body length,
\[
  R = \frac{L_\mathrm{rect}}{2\sin(\theta/2)},\qquad
  L_\mathrm{ref} = R\theta
    = L_\mathrm{rect}\frac{\theta/2}{\sin(\theta/2)}
    = 1.0261721530\ \mathrm{m}.
\]
The OPALX body pose is the rectangular body frame.  With the requested entrance
edge at
\[
  \vect r_0=(X_0,0,Z_0),\qquad X_0=0,\qquad Z_0=d,
\]
...
\[
  \vect q_\mathrm{entry}
    ...
    \end{pmatrix}.
\]
Using the same \(X\)-\(Z\) rotation convention as the OPALX input files,
\[
  \mathcal R_Y(\Theta)
...
survey.  The physics comparison is not yet at the SBEND level.  In the
\code{DesignPath.dat} comparison the largest orbit differences are
\(6.09\times10^{-2}\ \mathrm{m}\) in \(X\) and
\(4.97\times10^{-2}\ \mathrm{m}\) in relative \(Z\).  The sampled
\(B_y\) peak is \(4.749971\) in OPALX and \(2.359859\) in OPAL, and the visible
field support starts at \(s_\mathrm{rel}=0\) in OPALX but near
\(0.201\ \mathrm{m}\) in OPAL.  This points to the RBEND field/local-coordinate
evaluation, not the exported element placement, as the remaining issue.
...
\(B_y\) peak is too large in OPALX and the momentum components diverge from
old OPAL across the bend.}
\end{figure}
...
\text{nominal exit edge}           & s_\mathrm{rel}=1.4261722 & z_\mathrm{rel}=1.4000000,\\
\text{downstream support end}      & s_\mathrm{rel}=1.7090149 & z_\mathrm{rel}=1.6000000 .
\end{array}
\]

The current RBEND vertical test does not yet provide a clean edge-focusing
regression, because the field discrepancy dominates the vertical dynamics.
The \code{DesignPath.dat} \(B_y\) peak is \(4.216923\) in OPALX and
\(2.359859\) in OPAL; the \code{.stat} reference-particle peak is
\(4.402652\) in OPALX and \(2.359859\) in OPAL.  At the final common
\code{.stat} sample the mean vertical positions are
\[
  \bar y_\mathrm{OPALX}=8.094384\times10^{-4}\ \mathrm{m},\qquad
  \bar y_\mathrm{OPAL}=8.804121\times10^{-4}\ \mathrm{m},
\]
for a final difference of \(-7.10\times10^{-5}\ \mathrm{m}\).  This case is
therefore useful as a diagnostic for the remaining RBEND field-coordinate
problem, but it should not be promoted to a strict regression until the
reference field agrees with old OPAL.
...
positions use the rectangular RBEND arc length for \(s_\mathrm{rel}\) and the
straight body length for \(z_\mathrm{rel}\).}
\end{figure}
...
\(\bar y\) column.  The visible vertical difference is measured while the
RBEND field magnitude and support still disagree with old OPAL.}
\end{figure}
