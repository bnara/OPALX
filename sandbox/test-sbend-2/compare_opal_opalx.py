#!/usr/bin/env python3
"""Compare OPALX and old OPAL SBEND output for test-sbend-2.
...
BEND_DRIFT_LENGTH = 0.4
BEND_LENGTH = 1.0
BEND_ANGLE = np.pi / 4.0
BEND_E1 = np.deg2rad(-49.99)
BEND_E2 = 0.0
BEND_GAP = 0.04
BEND_ENGE_HALF_WIDTH = 5.0 * BEND_GAP


def bend_geometry_markers(*, coordinate: str) -> dict[str, float]:
    radius = BEND_LENGTH / (2.0 * np.sin(BEND_ANGLE / 2.0))
    body_arc_length = radius * BEND_ANGLE
    entry_support = BEND_ENGE_HALF_WIDTH / abs(np.cos(BEND_E1))
    exit_support = BEND_ENGE_HALF_WIDTH / np.cos(BEND_E2)
    nominal_entry = BEND_DRIFT_LENGTH

    if coordinate == "s":
        nominal_exit = nominal_entry + body_arc_length
        downstream_support_end = nominal_exit + exit_support
    elif coordinate == "z":
        nominal_exit = nominal_entry + radius * np.sin(BEND_ANGLE)
        downstream_support_end = nominal_exit + exit_support * np.cos(BEND_ANGLE)
    else:
        raise ValueError(f"Unsupported marker coordinate: {coordinate}")

    return {
        "upstream Enge support start": nominal_entry - entry_support,
        "nominal entrance edge": nominal_entry,
        "nominal exit edge": nominal_exit,
        "downstream Enge support end": downstream_support_end,
    }


def add_bend_markers(ax, *, coordinate: str) -> None:
    markers = bend_geometry_markers(coordinate=coordinate)
    marker_styles = {
        "upstream Enge support start": {"color": "0.35", "linestyle": ":", "linewidth": 1.1},
        "nominal entrance edge": {"color": "0.05", "linestyle": "--", "linewidth": 1.1},
        "nominal exit edge": {"color": "0.05", "linestyle": "-.", "linewidth": 1.1},
    }
    for label, style in marker_styles.items():
        ax.axvline(markers[label], label=label, alpha=0.75, **style)
...
def write_plot(
    ...
    add_bend_markers(ax, coordinate="z")
    ax.set_xlabel("z relative to line start [m]")
    ax.set_ylabel("x [m]")
    ax.set_title("test-sbend-2 design path")
    ax.legend()
    fig.tight_layout()
    ...
    add_bend_markers(ax, coordinate="s")
    ax.set_xlabel("s relative to line start [m]")
    ax.set_ylabel("normalized momentum")
    ax.set_title("test-sbend-2 design momentum")
    ax.legend(ncol=2)
    fig.tight_layout()
    ...
    ax.set_title("test-sbend-2 design-path sampled field")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_xlabel("s relative to line start [m]")
    ax.set_ylabel("sampled vertical B field")
    ax.set_title("test-sbend-2 reference-particle sampled field")
    ax.legend()
    fig.tight_layout()
    ...
    ax.plot(opalx_stat["s_rel"], 1.0e3 * opalx_stat["mean_y"], label="OPALX")
    ax.plot(opal_stat["s_rel"], 1.0e3 * opal_stat["mean_y"], "--", label="OPAL 2022.1")
    add_bend_markers(ax, coordinate="s")
    ax.axhline(0.0, color="black", linewidth=0.8, alpha=0.45)
    ax.axhline(1.0, color="gray", linewidth=0.8, linestyle=":", alpha=0.55, label="initial y")
    ax.set_xlabel("s relative to line start [m]")
    ax.set_ylabel("vertical particle position y [mm]")
    ax.set_title("test-sbend-2 vertical particle motion")
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "vertical_particle_motion.png")
    plt.close(fig)
...
def main() -> int:
    ...
    opalx_design = read_design_path(args.opalx_dir / "data/test-sbend-2_DesignPath.dat")
    opal_design = read_design_path(args.opal_dir / "data/test-sbend-2_opal_DesignPath.dat")
    opalx_stat = read_sdds_stat(args.opalx_dir / "test-sbend-2.stat")
    opal_stat = read_sdds_stat(args.opal_dir / "test-sbend-2_opal.stat")
    ...
    geometry_markers = pd.DataFrame(
        [
            {
                "marker": marker,
                "s_rel": bend_geometry_markers(coordinate="s")[marker],
                "z_rel": bend_geometry_markers(coordinate="z")[marker],
            }
            for marker in bend_geometry_markers(coordinate="s")
        ]
    )
    ...
    geometry_markers.to_csv(args.out_dir / "geometry_markers.csv", index=False)
    if args.plots:
        write_plot(opalx_design, opal_design, opalx_stat, opal_stat, args.out_dir)

    summary = "\n".join(
        [
            "# test-sbend-2 OPALX vs OPAL 2022.1",
            "",
            ...
            "",
            "## Bend geometry markers",
            "",
            "The plotted marker lines separate the full OPAL Enge support from the nominal bend edges. "
            "The B-field support starts reported below use a `1e-6` peak-field threshold, so they mark the visible fringe rather than the mathematical start of the support.",
            "",
            markdown_table(geometry_markers),
            "",
            "## Design path",
            "",
