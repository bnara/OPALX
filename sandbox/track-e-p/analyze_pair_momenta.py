...
where px, py, pz are beta*gamma = p/(m_e c).  By default the script prints
numeric direction statistics only.  Use --plot to also write diagnostic
figures; plotting is kept optional because some macOS/font-cache environments
can fail while initializing matplotlib.
"""
...
def read_fromfile(path: Path) -> pd.DataFrame:
    ...
    df = pd.read_csv(path, sep=r"\s+", skiprows=2, names=COLUMNS)
...
def write_plots(frames: dict[str, pd.DataFrame], out: Path, pair_index: Optional[int]) -> list[Path]:
    """Write two PNG pages plus a two-page PDF diagnostic."""
    ...
    return [out, geometry_out, pdf_out]
