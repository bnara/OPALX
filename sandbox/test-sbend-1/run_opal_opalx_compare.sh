...
run_root="${1:-/tmp/opalx-sbend-compare}"
...
cp "${script_dir}/test-sbend-1.in" \
   "${script_dir}/test-sbend-1_distribution.txt" \
   "${opalx_dir}/"
cp "${script_dir}/test-sbend-1_opal.in" \
   "${script_dir}/test-sbend-1_opal_distribution.txt" \
   "${opal_dir}/"

(
    cd "${opalx_dir}"
    "${opalx_exe}" test-sbend-1.in
)
