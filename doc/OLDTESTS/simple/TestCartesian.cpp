...
int main(int argc, char *argv[])
{
  ...
  MeshBC_E mbc[2*D];
  for (unsigned b=0; b < (2*D); b++) mbc[b] = Reflective;
  ...
  mesh.storeSpacingFields(); // Div() fails without this
  BareField<Vektor<double,D>,D>& vertSpacings = *(mesh.VertSpacings);
  BareField<Vektor<double,D>,D>& cellSpacings = *(mesh.CellSpacings);
  // Dummy operations to avoid compiler warnings about not using these:
  NDIndex<D> zeroIndex; for (d=0; d<D; d++) zeroIndex[d] = Index(0,0,1);
  double food = vertSpacings[zeroIndex].get()(0);
  food = cellSpacings[zeroIndex].get()(0);
 ...
 * $Revision: 1.1.1.1 $   $Date: 2000/11/30 21:04:56 $
 * IPPL_VERSION_ID: $Id: TestCartesian.cpp,v 1.1.1.1 2000/11/30 21:04:56 adelmann Exp $ 
 ***************************************************************************/
