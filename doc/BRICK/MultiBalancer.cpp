...
void MultiBalancer::distribute()
{
  ...
  vector<int> vnodeIndirect(m_vnodes);
  ...
  for (m=0; m<m_materials; ++m)
    m_materialMaxWeights[m] = 0.0;
  ...
  for (int i=m_vnodes-1; i>=0; --i) 
    {
      ...
      for (int m=0; m<m_materials; ++m)
        {
          // Accumulate the weight into the weight for this proc.
          m_materialProcWeights[m][0][proc] += m_inputWeights[m][0][vnode];

          // See if this proc is now the maximum weight for this material.
          if ( m_materialMaxWeights[m] < m_materialProcWeights[m][0][proc] )
            m_materialMaxWeights[m] = m_materialProcWeights[m][0][proc];
        }
    }
...
int MultiBalancer::findProcForVnode( int v )
{
      ...
      for (int m=0; m<m_materials; ++m)
        {
          ...
          double w = m_materialProcWeights[m][0][p]+m_inputWeights[m][0][v];

          // If the weight is less than the maximum weight on other
          // procs, then the other proc determines the weight.
          if ( w < m_materialMaxWeights[m] )
            w = m_materialMaxWeights[m];
...
//
void
MultiBalancer::calcIndirectSort(vector<int>& vnodeIndirect,
