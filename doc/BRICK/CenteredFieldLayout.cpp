		   ...
		   int vnodes)
{
  ...
  for (int d=0; d<Dim; d++) 
    ndi[d] = Index(mesh.gridSizes[d] - 1);
  cfl.initialize(ndi, bi, edt, vnodes);
}
*/
//------------------Vert centering---------------------------------------------
template<unsigned Dim, class Mesh>
...
{
  NDIndex<Dim> ndi;
  for (int d=0; d<Dim; d++) ndi[d] = Index(mesh.gridSizes[d] - 1);
...
{
  NDIndex<Dim> ndi;
  for (int d=0; d<Dim; d++) ndi[d] = Index(mesh.gridSizes[d]);
  cfl.initialize(ndi, edt, vnodesAlongDirection, recurse, vnodes);
}
...
template<unsigned Dim, class Mesh, class Centering>
CenteredFieldLayout<Dim,Mesh,Centering>::
CenteredFieldLayout(BrickInfo *bi, Mesh& mesh, 
		    e_dim_tag *p,
		    int vnodes)
{
  TAU_TYPE_STRING(taustr, CT(*this) +  "void (" 
  ...
  PInsist(Dim<=Mesh::Dimension,
    "CenteredFieldLayout dimension cannot be greater than Mesh dimension!!");
  centeredInitialize(*this, mesh, p, bi, vnodes);
}
*/
