...
template<unsigned Dim>
void
FieldLayout<Dim>::initialize(const NDIndex<Dim>& domain, BrickInfo *bi,
			     e_dim_tag *p, int vnodes) {
  TAU_TYPE_STRING(taustr, "void (" + CT(domain) + ", e_dim_tag*, int)");
  TAU_PROFILE("FieldLayout::initialize()", taustr, TAU_LAYOUT);
  setup(domain, bi, p, vnodes);
}
*/
			     ...
			     const int *nbegin, const int *nend)
{
  ...
  for (i = 0; i < vnodes; ++i) {
    ...
    PInsist(nbegin[i] >= 0 && nbegin[i] < Ipl::getNodes(),
			...
			e_dim_tag *userflags, int vnodes)
{
  ...
  int nprocs = Ipl::getNodes();
  int myproc = Ipl::myNode();
			...
			e_dim_tag *userflags, int vnodes)
{
  ...
  Inform msgall("FieldLayout<Dim>::setup ", INFORM_ALL_NODES);


  /*

  Restrictions: Layout all parallel the number of vnodes is set to the
                number of partitions.

		The domain partition is stored in the BrickInfo class.
  ...
  */
  ...
  unsigned int nxl,nyl,nzl;
  unsigned int nxh,nyh,nzh;
  
  // Find the number processors.
  int nprocs = Ipl::getNodes();
  int myproc = Ipl::myNode();

  vnodes = bi->getNumberOfBricks(); // nprocs;
  
  NDIndex<Dim> *domains_c = new NDIndex<Dim>[vnodes];
  
  bi->getOrigin(&xmin,&ymin,&zmin);
  for (int v=0; v<vnodes; v++) {
    
    bi->getLowerIndex(v,xmin,ymin,zmin,&nxl,&nyl,&nzl);
    bi->getUpperIndex(v,xmin,ymin,zmin,&nxh,&nyh,&nzh);
    
    NDIndex<Dim> localDomain;
    
    localDomain[0] = Index(nxl,nxh);
    localDomain[1] = Index(nyl,nyh);
    localDomain[2] = Index(nzl,nzh);
    domains_c[v] = localDomain;
  }
  ...
  : Domain(domain)
{
  ...
  int tag = Ipl::Comm->next_tag( F_REPARTITION_BCAST_TAG , F_TAG_CYCLE );
  int node_count = Ipl::Comm->broadcast_others(bcast_mess,tag);
  ...
  while ((--node_count)>=0)
    {
      ...
      Message *recv_mess = Ipl::Comm->receive_block(other_node,tag);
      PAssert(recv_mess != 0);
...
bool FieldLayout<Dim>::read(const char *filename) {
  ...
  int tag = Ipl::Comm->next_tag(F_LAYOUT_IO_TAG, F_TAG_CYCLE);
  ...
  if (Ipl::myNode() == 0) {
	...
	// So, the number of tokens should be a multiple of three
	...
	for (int v=0; v < fvnodes; ++v) {
      ...
      Ipl::Comm->broadcast_others(msg, tag);
    }
  } else {
    // on the client nodes, get the info from node 0 and store it
    // first receive the message
    int node = 0;
    Message *msg = Ipl::Comm->receive_block(node, tag);
    PAssert(msg);
  ...
  int localvnodes = 0;
  for (int v=0; v < fvnodes; ++v) {
    int node = vnlist[v].getNode() % Ipl::getNodes();
    if (node == Ipl::myNode()) {
      if (v > localvnodes) {
