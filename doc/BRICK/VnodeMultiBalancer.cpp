...
void VnodeMultiRepartition(FieldLayout<Dim>& layout, 
  ...
  typename vector<BareField<bool,Dim>* >::iterator bfi;
  ...
  int pe = Ipl::Comm->myNode();   // My processor ID
  Message *msg;
  int partialTag  = Ipl::Comm->next_tag(VNMB_PARTIAL_TAG, VNMB_TAG_CYCLE);
  int completeTag = Ipl::Comm->next_tag(VNMB_COMPLETE_TAG, VNMB_TAG_CYCLE);
  if (pe == 0) {
    ...
    while (notReceived > 0) {
      ...
      int tag = partialTag;
      Message* msg2 = Ipl::Comm->receive_block(otherPE, tag);
    ...
    Ipl::Comm->broadcast_others(msg, completeTag);
    delete [] anyTrueElementsPartial;
  } else {
    // Send my partially-filled array to PE 0:
    msg = new Message();
    msg->put(anyTrueElements, anyTrueElements + vnodes);
    Ipl::Comm->send(msg, 0, partialTag);
    // Receive fully-filled array from PE 0:
    int pe0 = 0;
    msg = Ipl::Comm->receive_block(pe0, completeTag);
  ...
  partialTag  = Ipl::Comm->next_tag(VNMB_PARTIAL_TAG, VNMB_TAG_CYCLE);
  completeTag = Ipl::Comm->next_tag(VNMB_COMPLETE_TAG, VNMB_TAG_CYCLE);
  Message* msg4;
  if (pe == 0) {
    // Receive partially-filled arrays from other PE's:
    ...
    // Broadcast fully-filled array to other PE's:
    Ipl::Comm->broadcast_others(msg4, completeTag);
    delete [] peOwnerPartial;
  } else {
    // Send my partially-filled array to PE 0:
    msg4 = new Message();
    msg4->put(peOwner, peOwner + vnodes);
    Ipl::Comm->send(msg4, 0, partialTag);
    // Receive fully-filled array from PE 0:
    int pe0 = 0;
    msg4 = Ipl::Comm->receive_block(pe0, completeTag);
    msg4->get(peOwner);
  }
  delete msg4;
  ...
  if (nZeroSizeVnodes != 0) {
    ...
    // Iterate through the BareFields in the container:
    vector<BareField<bool,Dim>* >::iterator bfi2;
	...
	notReceived--;
      ...
      // Broadcast fully-filled array to other PE's:
      Ipl::Comm->broadcast_others(msg3, completeTag);
      delete [] anyTrueElementsPartial;
    } else {
      // Send my partially-filled array to PE 0:
      msg3 = new Message();
      msg3->put(anyTrueElements, anyTrueElements + vnodes);
      Ipl::Comm->send(msg3, 0, partialTag);
      // Receive fully-filled array from PE 0:
      int pe0 = 0;
      msg3 = Ipl::Comm->receive_block(pe0, completeTag);
