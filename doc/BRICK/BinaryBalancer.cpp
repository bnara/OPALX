           ...
           BareField<double,Dim>& weights, int tag)
{
  ...
  vector<double*> reducedBuffer;
  vector<Index> domainBuffer;
      ...
      if ( nrdomains>0 )
        {
          ...
          vector<Index>::iterator dbp = domainBuffer.begin();
          vector<double*>::iterator rbp = reducedBuffer.begin();
	      ...
	      int& cutLoc, int& cutAxis)
{
  ...
  vector<double> reduced(length);
  vector<double> subReduced(length);
  for (i=0; i<length; ++i)
    reduced[i] = 0;
  ...
  int expected = 0;
  int nodes = Ipl::getNodes();
  int mynode = Ipl::myNode();
  ...
  while ( --expected >= 0 )
    {
      ...
      Message *mess = Ipl::Comm->receive_block(any_node,reduce_tag);
      PAssert(mess != 0);
...
BcastCuts(int cutLoc, int cutAxis, int bcast_tag)
{
  ...
  mess->put(cutAxis);
  // Send it out.
  Ipl::Comm->broadcast_all(mess,bcast_tag);
}
...
template<unsigned Dim>
static void
ReceiveCuts(vector< NDIndex<Dim> > &domains,
	    vector< int >& nprocs,
	    int bcast_tag)
{
  ...
  vector< NDIndex<Dim> > cutDomains(nDomains*2);
  vector<int> cutProcs(vector<int>::size_type(nDomains*2));
  ...
  for (int expected = 0; expected < nDomains; ++expected)
    {
      ...
      Message *mess = Ipl::Comm->receive_block(whichDomain,bcast_tag);
      PAssert(mess != 0);
...
template<unsigned Dim>
static void
CutEach(vector< NDIndex<Dim> >& domains,
	vector< int >& nprocs,
	BareField<double,Dim>& weights)
{
  ...
  int reduce_tag = Ipl::Comm->next_tag( F_REDUCE_PERP_TAG , F_TAG_CYCLE );
  int bcast_tag  = Ipl::Comm->next_tag( F_REDUCE_PERP_TAG , F_TAG_CYCLE );
...
// Build a list of domains as we go.
  vector< NDIndex<Dim> > domains; // used by TAU_TYPE_STRING
  vector<int> procs;
  ...
  /*out << "Starting CalcBinaryRepartition, outstanding msgs="
      << Ipl::Comm->getReceived()
      << endl;*/
    
  // Get the processors we'll be dealing with.
  int nprocs = Ipl::Comm->getNodes();
  int myproc = Ipl::Comm->myNode();
