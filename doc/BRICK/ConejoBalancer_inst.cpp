...
:
  ...
  m_procs( IplInfo::getNodes() ),
  // Record which processor is mine.
  m_myProc( IplInfo::myNode() )
{
  ...
  for ( int proc=0; proc < m_procs ; ++proc )
    {
      ...
      Message *msg = Ipl::Comm->receive_block(proc,tag);
  ...
  for ( int sourceProc = 0; sourceProc < m_procs; ++sourceProc )
    {
      ...
      // Send the message.
      Ipl::Comm->send(msg,sourceProc,tag);
...
ConejoBalancer::receiveVnodesToSend(vector<int>& vnodeDestinations, int tag)
{
  ...
  int proc_zero = 0;
  Message *msg = Ipl::Comm->receive_block(proc_zero,tag);
