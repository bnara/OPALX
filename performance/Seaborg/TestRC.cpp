...
int main(int argc, char *argv[])
{
  ...
  int d;
  int Parent = 0;
  int tag = Pooma::Comm->next_tag(POOMA_APP_TAG0);
  double pi = acos(-1.0);
  double twopi = 2.0*pi;
  // Timer:
  Timer timer;
  ...
  while (true) {
	...
	Pooma::Comm->broadcast_all(mess, tag);
      }
    }
    // now each node recieves the data
    unsigned pe = Pooma::Comm->myNode();
    Message *mess = Pooma::Comm->receive_block(Parent, tag);
    PAssert(mess);
...
/***************************************************************************
 * $RCSfile: TestFFT.cpp,v $   $Author: zippy $
 * $Revision: 1.10 $   $Date: 1999/01/19 05:45:21 $
 ***************************************************************************/
