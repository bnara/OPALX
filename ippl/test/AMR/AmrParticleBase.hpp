...
void AmrParticleBase::CIC_Cells_Fracs_Basic (const ParticleBase& p,
					     const Real*         plo,
					     const Real*         dx,
					     Real*               fracs,
					     IntVect*            cells)
{
    ...
    AmrParticleBase::CIC_Fracs(frac, fracs);
    AmrParticleBase::CIC_Cells(cell, cells);
}

int
AmrParticleBase::CIC_Cells_Fracs (const AmrParticleBase& p,
                               const Real*         plo,
                               const Real*         dx_geom,
                               const Real*         dx_part,
                               Array<Real>&        fracs,
                               Array<IntVect>&     cells)
{
    ...
    if (dx_geom == dx_part)
    {
        ...
        fracs.resize(M);
        cells.resize(M);
        AmrParticleBase::CIC_Cells_Fracs_Basic(p,plo,dx_geom,fracs.dataPtr(),cells.dataPtr());
        return M;
    }
    //
    // The first element in fracs and cells is the lowest corner, the last is the highest.
    //
    const Real hilen[BL_SPACEDIM] = { D_DECL((p.R[0]-plo[0]+dx_part[0]/2)/dx_geom[0],
                                             (p.R[1]-plo[1]+dx_part[1]/2)/dx_geom[1],
                                             (p.R[2]-plo[2]+dx_part[2]/2)/dx_geom[2]) };

    const Real lolen[BL_SPACEDIM] = { D_DECL((p.R[0]-plo[0]-dx_part[0]/2)/dx_geom[0],
                                             (p.R[1]-plo[1]-dx_part[1]/2)/dx_geom[1],
                                             (p.R[2]-plo[2]-dx_part[2]/2)/dx_geom[2]) };
...
bool
AmrParticleBase::CrseToFine (const BoxArray&       cfba,
...
{
    BL_PROFILE("AmrParticleBase::CrseToFine()");
    //
...
bool
AmrParticleBase::FineToCrse (const AmrParticleBase&                p,
                          int                                flev,
                          ...
                          std::vector< std::pair<int,Box> >& isects)
{
    ...
    AmrParticleBase::CIC_Cells_Fracs(p, cgm.ProbLo(), cgm.CellSize(), cfracs, ccells);
...
void
AmrParticleBase::FineCellsToUpdateFromCrse (const AmrParticleBase&                p,
                                         int                                lev,
                                         const ParGDBBase*                  gdb,
                                         const IntVect&                     ccell,
                                         ...
                                         std::vector< std::pair<int,Box> >& isects)
{
    BL_PROFILE("AmrParticleBase::FineCellsToUpdateFromCrse()");
    BL_ASSERT(lev >= 0);
    ...
    for (IntVect iv = fbx.smallEnd(); iv <= fbx.bigEnd(); fbx.next(iv))
    {
        ...
        for (int k = 0; k < BL_SPACEDIM; k++)
        {
            ...
            if ((p.R[k] < celllo) && (celllo > (p.R[k] + dx[k]/2)))
                touches = false;

            if ((p.R[k] > cellhi) && (cellhi < (p.R[k] - dx[k]/2)))
                touches = false;
        }
    ...
    for (int j = 0; j < fcells.size(); j++)
    {
        ...
        for (int k = 0; k < BL_SPACEDIM; k++)
        {
            ...
            if (p.R[k] <= celllo)
            {
                const Real isecthi = p.R[k] + dx[k]/2;
            ...
            else
            {
                ...
                const Real isectlo = p.R[k] - dx[k]/2;
...
long
AmrParticleBase::MaxParticlesPerRead ()
{
    //
...
#pragma omp atomic capture
	...
	BoxLib::Abort("AmrParticleBase::NextID() -- too many particles");

    return next;
}

int
AmrParticleBase::UnprotectedNextID ()
{
...
void
AmrParticleBase::NextID (int nextid)
{
    the_next_id = nextid;
}

bool 
AmrParticleBase::SingleLevelWhere (AmrParticleBase& p, 
                                const ParGDBBase*    gdb,
                                int           level)
{
    BL_ASSERT(gdb != 0);

    const IntVect& iv = AmrParticleBase::Index(p,gdb->Geom(level));
...
bool
AmrParticleBase::PeriodicShift (AmrParticleBase& p,
                             const ParGDBBase*    gdb)
{
    //
    ...
    const IntVect&  iv      = AmrParticleBase::Index(p,gdb->Geom(0));
    bool            shifted = false;  
    ...
    for (int i = 0; i < BL_SPACEDIM; i++)
    {
        ...
        if (iv[i] > dmn.bigEnd(i))
        {
            if (p.R[i] == geom.ProbHi(i))
                ...
                //
                p.R[i] += .125*geom.CellSize(i);

            p.R[i] -= geom.ProbLength(i);

            if (p.R[i] <= geom.ProbLo(i))
                //
                // This can happen due to precision issues.
                //
                p.R[i] += .125*geom.CellSize(i);

            BL_ASSERT(p.R[i] >= geom.ProbLo(i));
...
void AmrParticleBase::Reset (AmrParticleBase& p,
			     const ParGDBBase*    gdb,
			     bool          update,
			     bool          verbose)
{
...
Real AmrParticleBase::InterpDoit (const FArrayBox& fab,
				  const IntVect&   cell,
				  const Real*      frac,
				  int              comp)
{
  ...
  Real val = AmrParticleBase::InterpDoit(fab,fracs,cells,comp);
...
void AmrParticleBase::Interp (const AmrParticleBase& prt,
			      const Geometry&     geom,
			      const FArrayBox&    fab,
			      const int*          idx,
			      Real*               val,
			      int                 cnt)
{
  ...
  // Get "fracs" and "cells".
  //
  AmrParticleBase::CIC_Cells_Fracs_Basic(prt, plo, dx, fracs, cells);

  for (int i = 0; i < cnt; i++) {
    BL_ASSERT(idx[i] >= 0 && idx[i] < fab.nComp());
    
    val[i] = AmrParticleBase::InterpDoit(fab,fracs,cells,idx[i]);
  }
}

void AmrParticleBase::GetGravity (const FArrayBox&    gfab,
				  const Geometry&     geom,
				  const AmrParticleBase& p,
				  Real*               grav)
{
    BL_PROFILE("AmrParticleBase::GetGravity()");
    BL_ASSERT(grav != 0);
    
    int idx[BL_SPACEDIM] = { D_DECL(0,1,2) };

    AmrParticleBase::Interp(p,geom,gfab,idx,grav,BL_SPACEDIM);
}
