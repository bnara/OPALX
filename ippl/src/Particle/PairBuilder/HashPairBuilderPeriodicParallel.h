		...
		template<class Pred, class OP>
			void for_each(const Pred& pred, const OP &op,Vektor<double,3> extend_l, Vektor<double,3> extend_r )
			{
				...
				for (unsigned i=0; i<3; ++i) {
					extend_l_local[i] = locDomain[i].first()*particles.hr_m[i]+extend_l[i];
					extend_r_local[i] = extend_l[i]+(locDomain[i].last()+1)*particles.hr_m[i];
				...
				dmsg << "done assigning particles to buckets" << endl;
