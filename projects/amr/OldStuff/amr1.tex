...
\title{Proposal for an interdepartmental \textbf{``ETH-CHIRP1''} project \\[5mm]
  \Huge Scalable Parallel Adaptive Mesh Refinement \\
  with Application to SwissFEL Beam Dynamics Simulations}

\author{%
  P. Arbenz (PA) (CCOS, D-INFK) \\
  A. Adelmann (AA) (AMAS-PSI)
}
...
In this project we will address applications in the area of particle accelerator science and more specific 
we focus on the SwissFEL. An important decision has been taken towards the realization of SwissFEL at PSI. 
The ETH Board has discussed PSI's proposal at its board meeting, beginning of March 2010, and has given unanimously 
its support by including the project in the strategic planning of the ETH Domain.

An important milestone for the realization of the new SwissFEL facility was reached on the 24th of August 2010, when the core of the new SwissFEL facility was set into operation at the Paul Scherrer Institute.
The 250 MeV injector is now in operation can be used to study in detail beam dynamics relevant for the SwissFEL and more over can be used to apply the techniques foreseen to be developed in this research proposal.
This is a unique opportunity to apply cutting edge numerical techniques on a test accelerator and make relevant and important contributions to the detailed understanding of beam dynamics.
...
large amounts of data.  The primary application that we target at in
this project, is the simulation of the Swiss Free Electron Laser
(SwissFEL). Billions of particles are accelerated by  electric fields
that are represented itself by billions of grid points in a mesh that
covers a large domain of complicated shape. 
