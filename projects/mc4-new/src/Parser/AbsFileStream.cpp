...
Token AbsFileStream::readToken()
{
  ...
  if (ch == '"'  ||  ch== '\'') {
    ...
    if ((neg=ispunct(ch)))
      curr_char++;
    return readNumber(neg);     // Numeric token.
  } else {
    // Delimiter tokens.
...
Token AbsFileStream::readNumber(bool neg)
{
  bool digit = false;
  bool eflag = false;
  double value = 0.0;
  ...
  if (digit  &&  line[curr_char] != '.'  &&
    ...
    if(neg)
      return Token(stream_name, curr_line, lexeme, -1.0*int(value+0.5));
    else
  ...
  if (eflag) {
    ...
    if(neg)
      return Token(stream_name, curr_line, lexeme, -1.0*value);
    else    
