...
namespace mslang {
    ...
    bool Intersection::parse_detail(iterator& it, const iterator& end, Function*& fun) {
        ...
        if (!parse(it, end, inter->firstOperand_m))
            return false;
        ...
        if (!std::regex_match(str, what, argumentList))
            return false;

        iterator it2 = it + 1;
        if (!parse(it2, end, inter->secondOperand_m))
            return false;

        it  = it2;
        str = std::string(it, end);
        if (!std::regex_match(str, what, endParenthesis))
            return false;
