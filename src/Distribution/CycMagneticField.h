...
//~
    ...
    CycMagneticField(const std::string fmapfn) : fmapfn_m(fmapfn) { }
...
//~
    ...
    */
    ...
    virtual double getSlices() const { return 1.0; }

    /// Get stepsize.
    //  Slices and stepsize used to determine integration step.
    virtual double getStepsize() const { return 1.0; }
...
{
//     std::cout << "Hallo" << std::endl; std::cin.get();
    // x horizontal
    // y longitudinal
    // z is vertical
...
//     tet_rad = theta / Physics::pi * 180.0;
    ...
    double tet_map = fmod(tet_rad, 360.0 / 12.0);//360.0 / 8.0/*symmetry_m*/);
...
//         std::cout << "if" << std::endl; std::cin.get();
        
//         std::cout << r1t1 << " " << r2t1 << " " << r1t2 << " " << r2t2 << std::endl;
