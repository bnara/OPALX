...
//     this->initCrseFineInterp_m(Interpolater::LAGRANGE);
    ...
    // base level solver
    const BaseSolver solver = this->convertToBaseSolver_m(bsolver);
    const Preconditioner precond = this->convertToPrec_m(prec);
...
AmrMultiGrid::convertToBaseSolver_m(const std::string& bsolver)
{
    ...
    if ( solver == map.end() )
        throw OpalException("AmrMultiGrid::convertToBaseSolver()",
                            "No such bottom solver.");
    return solver.second;
}


AmrMultiGrid::Preconditioner
AmrMultiGrid::convertToPrec_m(const std::string& prec)
{
    std::map<std::string, Preconditioner>& map
    
    
    auto precond = map.find(bsolver);
    
    if ( precond == map.end() )
        throw OpalException("AmrMultiGrid::convertToBaseSolver()",
                            "No such preconditioner.");
