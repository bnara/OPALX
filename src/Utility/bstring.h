...
// This is a locally modified version of Modena's old bstring class,
// attempting to make it as standard-compliant as possible.
...
struct  string_char_baggage {
    ...
    static std::ostream&
    char_out (std::ostream& os, char_type c) _THROW_NONE
    {
...
public:
    ...
    basic_string<charT>&
    erase (size_t pos = 0, size_t n = NPOS) _THROW_ALLOC_OUTRANGE ;
    ...
    friend
    std::ostream&
    operator<< (std::ostream& o, const basic_string<charT>& s) _THROW_NONE ;
...
template <class charT>
basic_string<charT>&
basic_string<charT>::erase (size_t pos, size_t n) _THROW_ALLOC_OUTRANGE
{
    if (pos > length())
    {
        reference_class::throwrange();
...
{
    // cast away const-ness so we can reset c_str_ptr
    basic_string<charT>* nc_this = (basic_string<charT>*) this;
    if (c_str_ptr)
        delete [] nc_this->c_str_ptr;
    nc_this->c_str_ptr = new charT [length()+1];
    if (length())
        baggage_type::copy (nc_this->c_str_ptr, data(), length());
    baggage_type::assign (*(nc_this->c_str_ptr+length()), eos());
    return c_str_ptr;
}
...
/***************************************************************************
 * $RCSfile: bstring.h,v $   $Author: adelmann $
 * $Revision: 1.1.1.1 $   $Date: 2003/01/23 07:40:34 $
 * IPPL_VERSION_ID: $Id: bstring.h,v 1.1.1.1 2003/01/23 07:40:34 adelmann Exp $ 
 ***************************************************************************/
