...
FM3DMagnetoStaticExtended::FM3DMagnetoStaticExtended(std::string aFilename)
    ...
    if (file.good()) {
        ...
        for (unsigned long i = 0; (i < (num_gridpz_m + 1) * (num_gridpx_m + 1)) && parsing_passed;
             ++i) {
        ...
        if (!parsing_passed) {
            ...
            throw GeneralClassicException(
                "FM3DMagnetoStaticExtended::FM3DMagnetoStaticExtended(std::string)",
    ...
    if (j == 1) {
        {
            ...
            for (unsigned int k = 0; k < num_gridpz_m; k++) {
                ...
                FieldstrengthBx_m[index] = 0.5 * hy_m / hx_m
        ...
        {  // treat cells at i = 0, j > 1, k = 0:num_gridpz_m - 1;
            ...
            for (unsigned int k = 0; k < num_gridpz_m; k++) {
                unsigned long index = getIndex(i, j, k);
                FieldstrengthBx_m[index] =
                    (4 * FieldstrengthBx_m[getIndex(i, j - 1, k)]
...
void FM3DMagnetoStaticExtended::integrateBz(unsigned j) {
    if (j == 1) {
        ...
        for (unsigned int i = 0; i < num_gridpx_m; i++) {
            for (unsigned int k = 1; k < num_gridpz_m - 1; k++) {
                unsigned long index      = getIndex(i, j, k);
                FieldstrengthBz_m[index] = 0.5 * hy_m / hz_m
        ...
        for (unsigned int i = 0; i < num_gridpx_m; i++) {
            for (unsigned int k = 1; k < num_gridpz_m - 1; k++) {
                ...
                FieldstrengthBz_m[index] =
                    (4 * FieldstrengthBz_m[getIndex(i, j - 1, k)]
    ...
    if (j == 1) {
        for (unsigned int i = 1; i < num_gridpx_m - 1; i++) {
            {  // treat cells at i = 1:num_gridpx_m - 2, j = 1, k = 0
                ...
                FieldstrengthBy_m[index] =
                    (FieldstrengthBy_m[getIndex(i, j - 1, k)]
                     + hy_m
                           * ((FieldstrengthBx_m[getIndex(i + 1, j, k)]
                               - FieldstrengthBx_m[getIndex(i - 1, j, k)])
                                  / hx_m
        ...
        }
        {
            ...
            {  // treat cells at i = 0, j = 1, k = 0
                ...
                FieldstrengthBy_m[index] =
                    (FieldstrengthBy_m[getIndex(i, j - 1, k)]
                     + hy_m
                           * ((-3 * FieldstrengthBx_m[getIndex(i, j, k)]
                               + 4 * FieldstrengthBx_m[getIndex(i + 1, j, k)]
                               - FieldstrengthBx_m[getIndex(i + 2, j, k)])
                                  / hx_m
                              - (-3 * FieldstrengthBz_m[getIndex(i, j, k)]
                                 + 4 * FieldstrengthBz_m[getIndex(i, j, k + 1)]
                                 - FieldstrengthBz_m[getIndex(i, j, k + 2)])
        ...
        for (unsigned int i = 1; i < num_gridpx_m - 1; i++) {
            {  // treat cells at i = 1:num_gridpx_m - 2, j > 1, k = 0
                ...
                unsigned long index = getIndex(i, j, k);
                FieldstrengthBy_m[index] =
                    (4 * FieldstrengthBy_m[getIndex(i, j - 1, k)]
...
void FM3DMagnetoStaticExtended::saveField(const std::string& fname, unsigned int j) const {
    ...
    for (unsigned int i = 0; i < num_gridpx_m; ++i, x += hx_m) {
        ...
        for (unsigned int k = 0; k < num_gridpz_m; ++k, z += hz_m) {
            unsigned long index = getIndex(i, j, k);
            out << std::setw(14) << x << std::setw(14) << y << std::setw(14) << z << std::setw(14)
                << FieldstrengthBx_m[index] << std::setw(14) << FieldstrengthBy_m[index]
...
Vector_t<double, 3> FM3DMagnetoStaticExtended::interpolateTrilinearly(
...
double FM3DMagnetoStaticExtended::getWeightedData(
...
bool FM3DMagnetoStaticExtended::getFieldstrength(
    const Vector_t<double, 3>& R, Vector_t<double, 3>& /*E*/, Vector_t<double, 3>& B) const {
    if (isInside(R)) {
        Vector_t<double, 3> suppB = interpolateTrilinearly(R);
...
bool FM3DMagnetoStaticExtended::getFieldDerivative(
...
void FM3DMagnetoStaticExtended::getFieldDimensions(
...
void FM3DMagnetoStaticExtended::getInfo(Inform* msg) {
    (*msg) << Filename_m << " (3D magnetostatic, extended); zini= " << zbegin_m
           << " m; zfinal= " << zend_m << " m;" << endl;
}
