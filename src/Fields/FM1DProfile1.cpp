    ...
    if (Filename_m == "1DPROFILE1-DEFAULT") {
        ...
        if (inputFile.good()) {
            ...
            bool parsingPassed = interpretLine<std::string, int, int, double>(
                inputFile, tempString, polyOrderEntry_m, polyOrderExit_m, gapHeight_m);

            parsingPassed = parsingPassed
                            && interpretLine<double, double, double, int>(
                                inputFile, entranceParameter1_m, entranceParameter2_m,
                                entranceParameter3_m, tempInt, false);

            parsingPassed =
                parsingPassed
                && interpretLine<double, double, double, int>(
                    inputFile, exitParameter1_m, exitParameter2_m, exitParameter3_m, tempInt);

            for (int index = 0; (index < polyOrderEntry_m + polyOrderExit_m + 2) && parsingPassed;
                 index++)
                parsingPassed = parsingPassed && interpretLine<double>(inputFile, tempDouble);
...
void FM1DProfile1::readMap() {
    ...
    if (Filename_m == "1DPROFILE1-DEFAULT") {
        ...
        interpretLine<std::string, int, int, double>(
            inputFile, tempString, tempInt, tempInt, tempDouble);
        interpretLine<double, double, double, int>(
            inputFile, tempDouble, tempDouble, tempDouble, tempInt);
        interpretLine<double, double, double, int>(
            inputFile, tempDouble, tempDouble, tempDouble, tempInt);
...
bool FM1DProfile1::getFieldstrength(
...
bool FM1DProfile1::getFieldDerivative(
    const Vector_t<double, 3>& /*R*/, Vector_t<double, 3>& /*E*/, Vector_t<double, 3>& /*B*/,
    const DiffDirection& /*dir*/) const {
    return false;
}

void FM1DProfile1::getFieldDimensions(double& sBegin, double& sEnd) const {
    sBegin = sBegin_m;
    sEnd   = sEnd_m;
}
void FM1DProfile1::getFieldDimensions(
...
void FM1DProfile1::getInfo(Inform* msg) {
    (*msg) << Filename_m << " (1D Profile type 1)" << endl;
}
...
void FM1DProfile1::get1DProfile1EngeCoeffs(
...
void FM1DProfile1::get1DProfile1EntranceParam(
...
void FM1DProfile1::get1DProfile1ExitParam(
