...
FM1DProfile2::FM1DProfile2(std::string aFilename)
    ...
    if (file.good()) {
        bool parsing_passed = interpretLine<std::string, int, int, double>(
            file, tmpString, polynomialOrder_entry_m, polynomialOrder_exit_m, gapHeight_m);
        parsing_passed =
            parsing_passed
            && interpretLine<double, double, double, int>(
                file, zbegin_entry_m, polynomialOrigin_entry_m, zend_entry_m, num_gridpz);
...
void FM1DProfile2::readMap() {
    if (EngeCoefs_entry_m == nullptr) {
        ...
        for (int i = 0; i < num_gridpz + 1; ++i) {
            ...
            if (RealValues[i] > maxValue)
                maxValue = RealValues[i];
            else if (RealValues[i] < minValue)
                minValue = RealValues[i];
        }
        in.close();
        ...
        int i = 0;
        while (i < num_gridpz + 1 && RealValues[i] < tolerance)
            ++i;
        num_gridp_before_fringe_entry = i - 1;

        // find end of entry fringe field //
        while (i < num_gridpz + 1 && RealValues[i] < 1. - tolerance)
            ++i;
        ...
        double first =
            polynomialOrigin_entry_m - zbegin_entry_m - dZ * (num_gridp_before_fringe_entry + 1);
        ...
        QRDecomposition::solve(
            leastSquareMatrix, EngeCoefs_entry_m, rightHandSide, num_gridp_fringe_entry,
            polynomialOrder_entry_m + 1);
        ...
        QRDecomposition::solve(
            leastSquareMatrix, EngeCoefs_exit_m, rightHandSide, num_gridp_fringe_exit,
            polynomialOrder_exit_m + 1);
...
bool FM1DProfile2::getFieldstrength(
    const Vector_t<double, 3>& R, Vector_t<double, 3>& strength, Vector_t<double, 3>& info) const {
    ...
    RExit(0) = (R(0) - xExit_m) * cosExitRotation_m
               - (R(2) + zbegin_entry_m - zExit_m) * sinExitRotation_m;
    RExit(2) = (R(0) - xExit_m) * sinExitRotation_m
               + (R(2) + zbegin_entry_m - zExit_m) * cosExitRotation_m + polynomialOrigin_exit_m;
    ...
    if (REntrance(2) >= zend_entry_m && RExit(2) <= zbegin_exit_m) {
        ...
        if (f > 1.e-30) {
            ...
            double d2fdz2 = ((-d2Sdz2 - dSdz * dSdz * (1. - 2. * (expS * f))) * (f * expS) * f)
                            / (gapHeight_m * gapHeight_m);
    ...
    //            polynomialOrder = polynomialOrder_entry_m;
    //            info(0) = 1.0;
    //        } else if (tmpR(2) > exit_slope_m * tmpR(0) + zbegin_exit_m && tmpR(2) <= exit_slope_m
    //        * tmpR(0) + zend_exit_m) {
    //            z = (tmpR(2) - exit_slope_m * tmpR(0) - polynomialOrigin_exit_m) /
    //            sqrt(exit_slope_m * exit_slope_m + 1) / gapHeight_m; EngeCoefs = EngeCoefs_exit_m;
    //            polynomialOrder = polynomialOrder_exit_m;
    //            info(0) = 2.0;
    //        } else {
    //            return true;
    ...
    //            d2fdz2 = ((-d2Sdz2 - dSdz * dSdz * (1. - 2. * (expS * f))) * (f * expS) * f) /
    //            (gapHeight_m * gapHeight_m);  // second derivative of f
    //
    //            strength(0) = f;
    //            strength(1) = dfdz / gapHeight_m;
    //            strength(2) = d2fdz2;
...
bool FM1DProfile2::getFieldDerivative(
...
void FM1DProfile2::getFieldDimensions(
...
void FM1DProfile2::getInfo(Inform* msg) {
    (*msg) << Filename_m << " (1D Profile type 2); zini= " << zbegin_entry_m
           << " m; zfinal= " << zend_exit_m << " m;" << endl;
}
...
void FM1DProfile2::setEdgeConstants(
...
namespace QRDecomposition {
    void solve(
        double* Matrix, double* Solution, double* rightHandSide, const int& M, const int& N) {
        double sinphi;
        double cosphi;
        double tempValue;
        double len;
