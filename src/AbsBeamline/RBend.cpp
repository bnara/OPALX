...
CoordinateSystemTrafo RBend::getEdgeToBegin() const {
    ...
    const Euclid3D bodyToReferenceEntry = Euclid3D::translation(xMid, 0.0, 0.5 * getElementLength())
                                          * Euclid3D::YRotation(halfAngle);
...
CoordinateSystemTrafo RBend::getEdgeToEnd() const {
    ...
    const Euclid3D referenceEntryToExit =
            Euclid3D::translation(
                    -getElementLength() * std::tan(halfAngle), 0.0, getElementLength())
