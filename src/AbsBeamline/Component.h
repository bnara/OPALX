...
public:
    ...
    //    virtual bool determineEntryPoint(const double &kineticEnergy, const double &tolerance) =
    //    0;
    ...
    // void getOrientation(Vector_t<double, 3> &, double &) const;  // first component is alpha,
    // second is beta. the third component is always neglected.
    // // alpha is defined as the angle between projection of the normal of the face of the element
    // onto
    // // the s-u plane and the s vector. for beta the following is valid:
    // // beta = arccos(|n_{parallel}|) where n_{parallel} is the projection of the normal onto the
    // s-u
    // // plane

    // void setOrientation(const Vector_t<double, 3> &direction);
...
// inline void Component::getOrientation(Vector_t<double, 3> &ori, double &m) const {
//     ori = Orientation_m;
//     m = exit_face_slope_m;
// }

// inline void Component::setOrientation(const Vector_t<double, 3> &direction)
