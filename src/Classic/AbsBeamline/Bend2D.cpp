...
bool Bend2D::apply(const Vector_t<double, 3> &R,
...
bool Bend2D::applyToReferenceParticle(const Vector_t<double, 3> &R,
                                    const Vector_t<double, 3> &P,
                                    const double &t,
                                    Vector_t<double, 3> &E,
                                    Vector_t<double, 3> &B) {
...
void Bend2D::initialise(PartBunch<double, 3> *bunch,
                      double &startField,
                      double &endField) {
...
double Bend2D::calculateBendAngle() {
    ...
    Vector_t<double, 3> P = betaGamma * Vector_t<double, 3>(sinEntranceAngle_m, 0.0, cosEntranceAngle_m);
    double deltaS = 0.0;
...
Vector_t<double, 3> Bend2D::calcCentralField(const Vector_t<double, 3> &/*R*/, double /*deltaX*/) {
    ...
    //double cosangle = dot(R - rotationCenter, Vector_t<double, 3>(1, 0, 0)) / euclidean_norm(R - rotationCenter);
...
Vector_t<double, 3> Bend2D::calcEntranceFringeField(const Vector_t<double, 3> &R,
                                         double /*deltaX*/) {

    const CoordinateSystemTrafo toEntranceRegion(Vector_t<double, 3>(0, 0, entranceParameter2_m),
                                                 Quaternion(0, 0, 1, 0));
...
Vector_t<double, 3> Bend2D::calcExitFringeField(const Vector_t<double, 3> &R,
                                     double /*deltaX*/) {

    const CoordinateSystemTrafo fromEndToExitRegion(Vector_t<double, 3>(0, 0, exitParameter2_m),
                                                    Quaternion(1, 0, 0, 0));
...
bool Bend2D::calculateMapField(const Vector_t<double, 3> &R, Vector_t<double, 3> &B) {
    ...
    Vector_t<double, 3> rotationCenter(-designRadius_m * cosEntranceAngle_m, R(1), designRadius_m * sinEntranceAngle_m);
...
void Bend2D::setFieldCalcParam() {
    ...
    Vector_t<double, 3> chord = getChordLength() * halfRotationAboutAxis.rotate(Vector_t<double, 3>(0, 0, 1));
    beginToEnd_lcs_m = CoordinateSystemTrafo(chord, exitFaceRotation.conjugate());
    beginToEnd_m = beginToEnd_lcs_m * CoordinateSystemTrafo(Vector_t<double, 3>(0.0), rotationAboutZ.conjugate());
    toEntranceRegion_m = CoordinateSystemTrafo(Vector_t<double, 3>(0, 0, entranceParameter2_m),
                                               Quaternion(0, 0, 1, 0));
    const CoordinateSystemTrafo fromEndToExitRegion(Vector_t<double, 3>(0, 0, exitParameter2_m),
                                                    Quaternion(1, 0, 0, 0));
    ...
    Vector_t<double, 3> rotationCenter = Vector_t<double, 3>(-designRadius_m * cosEntranceAngle_m, 0.0, designRadius_m * sinEntranceAngle_m);
    ...
    if (getBeginToEnd_local().transformTo(rotationCenter)(2) > 0.0) {
        double tau, tmp;
        Vector_t<double, 3> P(0.5 * aperture_m.second[0], 0.0, 0.0), R = getBeginToEnd_local().transformTo(rotationCenter);
        gsl_poly_solve_quadratic(dot(P,P),
                                 -2 * dot(P,R),
                                 dot(R,R) - std::pow(designRadius_m + 0.5 * aperture_m.second[0], 2),
                                 &tau,
...
void Bend2D::setupPusher(PartBunch<double, 3> *bunch) {
...
std::vector<Vector_t<double, 3>> Bend2D::getOutline() const {
    ...
    outline.push_back(Vector_t<double, 3>(-0.5 * widthEntranceFringe_m + entranceParameter1_m * tanEntranceAngle_m, 0.0, entranceParameter1_m));
    outline.push_back(Vector_t<double, 3>(entranceParameter1_m * tanEntranceAngle_m, 0.0, entranceParameter1_m));
    outline.push_back(Vector_t<double, 3>(0.5 * widthEntranceFringe_m + entranceParameter1_m * tanEntranceAngle_m, 0.0, entranceParameter1_m));
    outline.push_back(Vector_t<double, 3>(0.5 * widthEntranceFringe_m, 0.0, 0.0));

    {
        ...
        Quaternion rotation = getQuaternion(upperCornerAtEntry - rotationCenter, Vector_t<double, 3>(0,0,1));
        Vector_t<double, 3> tmp = CoordinateSystemTrafo(rotationCenter, rotation).transformTo(upperCornerAtExit);
    ...
    outline.push_back(getBeginToEnd_local().transformFrom(Vector_t<double, 3>(0.5 * widthExitFringe_m, 0.0, 0.0)));
    outline.push_back(getBeginToEnd_local().transformFrom(Vector_t<double, 3>(0.5 * widthExitFringe_m - exitParameter3_m * tanExitAngle_m, 0.0, exitParameter3_m)));
    outline.push_back(getBeginToEnd_local().transformFrom(Vector_t<double, 3>(-exitParameter3_m * tanExitAngle_m, 0.0, exitParameter3_m)));
    outline.push_back(getBeginToEnd_local().transformFrom(Vector_t<double, 3>(-0.5 * widthExitFringe_m - exitParameter3_m * tanExitAngle_m, 0.0, exitParameter3_m)));
    outline.push_back(getBeginToEnd_local().transformFrom(Vector_t<double, 3>(-0.5 * widthExitFringe_m, 0.0, 0.0)));

    {
        ...
        Quaternion rotation = getQuaternion(lowerCornerAtEntry - rotationCenter, Vector_t<double, 3>(0,0,1));
        Vector_t<double, 3> tmp = CoordinateSystemTrafo(rotationCenter, rotation).transformTo(lowerCornerAtExit);
        double totalAngle = -std::fmod(Physics::two_pi - std::atan2(tmp(0), tmp(2)), Physics::two_pi);
        double dAngle = 0.5 * totalAngle / (1.0 * numSteps - 1.0);
...
MeshData Bend2D::getSurfaceMesh() const {
    ...
    Vector_t<double, 3> rotationCenter(-designRadius_m * cosEntranceAngle_m, 0.0, designRadius_m * sinEntranceAngle_m);

    Vector_t<double, 3> P1 = toEntranceRegion_m.transformFrom(Vector_t<double, 3>(0, 0, entranceParameter1_m));
    Vector_t<double, 3> P2 = toExitRegion_m.transformFrom(Vector_t<double, 3>(0, 0, exitParameter1_m));
    ...
    if (T[1] > 0) { // fringe fields are overlapping
        ...
        if (this->getType() == ElementType::RBEND ||
            ...
            Quaternion halfRot(std::cos(0.25 * angle), std::sin(0.25 * angle) * Vector_t<double, 3>(0, 1, 0));
        ...
        gsl_poly_solve_quadratic(dot(P,P),
                                 ...
                                 &tau2);
        tau1 = (std::abs(1.0 - tau2) < std::abs(1.0 - tau1)? tau2: tau1);
        Vector_t<double, 3> lowerCornerFringeLimitEntrance = Vector_t<double, 3>(0, 0, entranceParameter3_m) + tau1 * P;
        ...
        gsl_poly_solve_quadratic(dot(P,P),
                                 ...
                                 &tau2);
        tau1 = (std::abs(1.0 - tau2) < std::abs(1.0 - tau1)? tau2: tau1);
        Vector_t<double, 3> upperCornerFringeLimitEntrance = Vector_t<double, 3>(0, 0, entranceParameter3_m) - tau1 * P;

        P = Vector_t<double, 3>(0.5 * aperture_m.second[0], 0.0, 0.0);
        R = Vector_t<double, 3>(0, 0, exitParameter1_m) - getBeginToEnd_local().transformTo(rotationCenter);
        gsl_poly_solve_quadratic(dot(P,P),
                                 2 * dot(P,R),
                                 dot(R,R) - std::pow(designRadius_m + 0.5 * aperture_m.second[0],2),
                                 &tau1,
                                 &tau2);
        tau1 = (std::abs(1.0 - tau2) < std::abs(1.0 - tau1)? tau2: tau1);
        Vector_t<double, 3> upperCornerFringeLimitExit = getBeginToEnd_local().transformFrom(Vector_t<double, 3>(0, 0, exitParameter1_m) + tau1 * P);
        ...
        gsl_poly_solve_quadratic(dot(P,P),
                                 ...
                                 &tau2);
        tau1 = (std::abs(1.0 - tau2) < std::abs(1.0 - tau1)? tau2: tau1);
        Vector_t<double, 3> lowerCornerFringeLimitExit = getBeginToEnd_local().transformFrom(Vector_t<double, 3>(0, 0, exitParameter1_m) - tau1 * P);
    ...
    mesh.decorations_m.push_back(std::make_pair(Vector_t<double, 3>(entranceParameter1_m * tanEntranceAngle_m, 0.0, entranceParameter1_m),
                                                Vector_t<double, 3>(0.0)));
    mesh.decorations_m.push_back(std::make_pair(getBeginToEnd_local().transformFrom(Vector_t<double, 3>(0.0)),
                                                getBeginToEnd_local().transformFrom(Vector_t<double, 3>(-exitParameter3_m * tanExitAngle_m, 0.0, exitParameter3_m))));
