...
template <
        ...
        , class Comm_t
>
class SPilot : protected Poller {

public:

    SPilot(CmdArguments_t args, boost::shared_ptr<Comm_t> comm)
        : Poller(comm->mpiComm())
        , comm_(comm)
        , cmd_args_(args)
    {
...
private:
    ...
    void parseInputFile() {
        ...
        // expressions
        functionDictionary_t known_expr_funcs; // let empty
        ...
        try {
            parser->doParse();
            parser->getProblem(objectives, constraints_, dvars_);
        } catch (OptPilotException &e) {
    ...
    void startWorker() {
        ...
        boost::scoped_ptr< SWorker<Sim_t> > w(
            new SWorker<Sim_t>(constraints_, simName,
                               comm_->getBundle(), cmd_args_));
    ...
    void startPilot() {
        ...
        if(num_coworkers_ > 1) {
            ...
            int alpha = cmd_args_->getArg<int>("nsamples", false);
            int opt_size = 1 + constraints_.size();
            int overhead = 10;
            ...
            master_node_.reset(
                new MasterNode< typename Opt_t::SolutionState_t,
                    ...
                    coworker_comm_, upperbound_buffer_size, 1,
...
//         std::cout << "SPilot::postPoll" << std::endl;
    ...
    void sendNewJobToWorker(int worker) {
        ...
        std::cout << "SPilot::sendNewJobToWorker" << std::endl;
...
bool onMessage(MPI_Status status, size_t recv_value){
    ...
    std::cout << "SPilot::onMessage" << std::endl;
...
//         MPI_Send_reqvars(res, opt_master_rank, opt_comm_);
        ...
        std::cout << "WORKER_FINISHED_TAG" << std::endl;
        
        std::cout << (request_queue_.size() > 0) << std::endl;
    ...
    case OPT_NEW_JOB_TAG: {
        ...
        std::cout << "SPilot: OPT_NEW_JOB_TAG" << std::endl;
