...
public:
    ...
    virtual void readStep(PartBunch<double, 3>*, h5_ssize_t firstParticle, h5_ssize_t lastParticle);

    virtual void writeHeader();
    virtual void writeStep(PartBunch<double, 3>*, const std::map<std::string, double>& additionalStepAttributes);

    virtual bool predecessorIsSameFlavour() const;

private:
    void readStepHeader(PartBunch<double, 3>*);
    void readStepData(PartBunch<double, 3>*, h5_ssize_t, h5_ssize_t);

    void writeStepHeader(PartBunch<double, 3>*, const std::map<std::string, double>&);
    void writeStepData(PartBunch<double, 3>*);
};
