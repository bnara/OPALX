...
void PartBins::sortArray() {
    /** sort the vector of particles such that position of the particles decrease with increasing
       index. Then push the particles back by 1e-13 s * beta * c (approximately one step). In order
       that the method getBin(double x) works xmin_m has to be lowered a bit more.
    */
...
Inform& PartBins::print(Inform& os) {
    ...
    os << "Bins= " << bins_m << " hBin= " << hBin_m << " Particle vector length "
       << tmppart_m.size() << endl;
