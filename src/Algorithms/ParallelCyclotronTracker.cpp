...
Vector_t<double, 3> const ParallelCyclotronTracker::xaxis = Vector_t<double, 3>(1.0, 0.0, 0.0);
Vector_t<double, 3> const ParallelCyclotronTracker::yaxis = Vector_t<double, 3>(0.0, 1.0, 0.0);
Vector_t<double, 3> const ParallelCyclotronTracker::zaxis = Vector_t<double, 3>(0.0, 0.0, 1.0);
...
ParallelCyclotronTracker::ParallelCyclotronTracker(
    ...
    bool revBeam, bool revTrack, int maxSTEPS, Steppers::TimeIntegrator timeintegrator,
    const int& numBunch, const double& mbEta, const double& mbPara, const std::string& mbMode,
    ...
    if (numBunch > 1) {
        mbHandler_m = std::unique_ptr<MultiBunchHandler>(
            new MultiBunchHandler(bunch, numBunch, mbEta, mbPara, mbMode, mbBinning));
    }
...
void ParallelCyclotronTracker::bgf_main_collision_test() {
    ...
    Vector_t<double, 3> intecoords = 0.0;
    ...
    for (size_t i = 0; i < itsBunch_m->getLocalNum(); i++) {
        int res = bgf_m->partInside(itsBunch_m->R[i], itsBunch_m->P[i], dtime, intecoords, triId);
        if (res >= 0) {
            lossDs_m->addParticle(
                OpalParticle(
                    itsBunch_m->ID[i], itsBunch_m->R[i], itsBunch_m->P[i], itsBunch_m->getT(),
                    itsBunch_m->Q[i], itsBunch_m->M[i]),
                std::make_pair(turnnumber_m, itsBunch_m->bunchNum[i]));
            itsBunch_m->Bin[i] = -1;
            Inform gmsgALL("OPAL", INFORM_ALL_NODES);
            gmsgALL << level4 << "* Particle " << itsBunch_m->ID[i] << " lost on boundary geometry"
                    << endl;
        }
    }
}
...
void ParallelCyclotronTracker::dumpAngle(
    const double& theta, double& prevAzimuth, double& azimuth, const short& bunchNr) {
...
double ParallelCyclotronTracker::computeRadius(const Vector_t<double, 3>& meanR) const {
...
void ParallelCyclotronTracker::openFiles(size_t numFiles, std::string SfileName) {
    for (unsigned int i = 0; i < numFiles; i++) {
        ...
        if (i <= 2) {
            ...
            // for single Particle Mode, output after each turn, to define matched initial phase
            // ellipse.
...
void ParallelCyclotronTracker::visitCyclotron(const Cyclotron& cycl) {
    *gmsg << "* ----------------------------- Cyclotron -------------------------------- *" << endl;
    ...
    if (spiral_flag) {
        *gmsg << endl
              << "* This is a Spiral Inflector Simulation! This means the following:" << endl;
        *gmsg << "* 1.) It is up to the user to provide appropriate geometry, electric and "
                 "magnetic fields!"
              << endl;
        *gmsg << "*     (Use BANDRF type cyclotron and use RFMAPFN to load both magnetic" << endl;
        *gmsg << "*     and electric fields, setting SUPERPOSE to an array of TRUE values.)"
              << endl;
        *gmsg << "* 2.) For high currents it is strongly recommended to use the SAAMG fieldsolver,"
              << endl;
        *gmsg << "*     FFT does not give the correct results (boundary conditions are missing)."
              << endl;
        *gmsg << "* 3.) The whole geometry will be meshed and used for the fieldsolver." << endl;
        *gmsg << "*     There will be no transformations of the bunch into a local frame und "
                 "consequently,"
              << endl;
        *gmsg << "*     the problem will be treated non-relativistically!" << endl;
        *gmsg << "*     (This is not an issue for spiral inflectors as they are typically < 100 "
                 "keV/amu.)"
              << endl;
        *gmsg << endl
              << "* Note: For now, multi-bunch mode (MBM) needs to be de-activated for spiral "
                 "inflector"
              << endl;
        *gmsg << "* and space charge needs to be solved every time-step. numBunch_m and "
                 "scSolveFreq are reset."
              << endl;
        if (isMultiBunch()) {
            mbHandler_m = nullptr;
        }
    ...
    if (!OpalData::getInstance()->inRestartRun()) {
        ...
        float insqrt =
            referencePtot * referencePtot - referencePr * referencePr - referencePz * referencePz;
        ...
        if (insqrt < 0) {
            if (insqrt > -1.0e-10) {
                ...
                throw OpalException(
                    "Error in ParallelCyclotronTracker::visitCyclotron", "Pt imaginary");
            }
        } else {
    ...
    *gmsg << "* RF frequency         = " << Util::doubleVectorToString(rffrequ) << " [MHz]" << endl;
    ...
    if (cycl_m->getBFieldType() == Cyclotron::BFieldType::BANDRF) {
        ...
        *gmsg << "* RF inital phase      = " << Util::doubleVectorToString(rfphi) << " [deg]"
              << endl;
        ...
        *gmsg << "* Superpose electric field maps -> " << Util::boolVectorToUpperString(superpose)
              << endl;
    }
...
void ParallelCyclotronTracker::visitMultipole(const Multipole& mult) {
    *gmsg << "In Multipole; L= " << mult.getElementLength() << " however the element is missing "
          << endl;
 ...
 * @param multTccurv
 */
void ParallelCyclotronTracker::visitMultipoleTCurvedConstRadius(
 ...
 * @param multTvcurv
 */
void ParallelCyclotronTracker::visitMultipoleTCurvedVarRadius(
    const MultipoleTCurvedVarRadius& multTvcurv) {
...
void ParallelCyclotronTracker::visitRBend(const RBend& bend) {
    *gmsg << "In RBend; L= " << bend.getElementLength() << " however the element is missing "
          << endl;
...
void ParallelCyclotronTracker::visitRing(const Ring& ring) {
    *gmsg << "* ----------------------------- Ring ------------------------------------- *" << endl;
    ...
    if (referenceTheta <= -180.0 || referenceTheta > 180.0) {
        throw OpalException(
            "Error in ParallelCyclotronTracker::visitRing", "PHIINIT is out of [-180, 180)");
    }
...
void ParallelCyclotronTracker::visitSBend(const SBend& bend) {
    *gmsg << "In SBend; L = " << bend.getElementLength() << " however the element is missing "
          << endl;
...
void ParallelCyclotronTracker::visitStripper(const Stripper& stripper) {
    ...
    *gmsg << std::boolalpha << "* Particles stripped will be deleted after interaction -> " << stop
          << endl;
 ...
 * @param elptr
 */
void ParallelCyclotronTracker::buildupFieldList(
...
void ParallelCyclotronTracker::execute() {
    ...
    for (auto fd : FieldDimensions) {
        ...
        *gmsg << "* -> " << ElementBase::getTypeString(type) << endl;
        if (type == ElementType::RFCAVITY) {
            ...
            CavityCrossData ccd = {
                cav, cav->getSinAzimuth(), cav->getCosAzimuth(), cav->getPerpenDistance()};
            cavCrossDatas_m.push_back(ccd);
        } else if (
            type == ElementType::CCOLLIMATOR || type == ElementType::PROBE
            || type == ElementType::SEPTUM || type == ElementType::STRIPPER) {
    ...
    *gmsg << "* ------------------------------------------------------------------------ *" << endl
    ...
    if (bgf_m)
        lossDs_m = std::unique_ptr<LossDataSink>(
            new LossDataSink(bgf_m->getOpalName(), !Options::asciidump));

    // External field arrays for dumping
    for (int k = 0; k < 2; k++)
        FDext_m[k] = Vector_t<double, 3>(0.0, 0.0, 0.0);

    extE_m = Vector_t<double, 3>(0.0, 0.0, 0.0);
    extB_m = Vector_t<double, 3>(0.0, 0.0, 0.0);
    DumpFields::writeFields((((*FieldDimensions.begin())->second).second));
    DumpEMFields::writeFields((((*FieldDimensions.begin())->second).second));

    function_t func = std::bind(
        &ParallelCyclotronTracker::getFieldsAtPoint, this, std::placeholders::_1,
...
void ParallelCyclotronTracker::MtsTracker() {
    ...
    *gmsg << "* MTS: The inner time step is therefore " << dt_inner << " [ns]" << endl;
    ...
    bool flagTransition =
        false;  // flag to determine when to transit from single-bunch to multi-bunches mode
    ...
    *gmsg << "* ---------------------- Start tracking ---------------------------------- *" << endl;
    ...
    for (; (step_m < maxSteps_m) && (itsBunch_m->getTotalNum() > 0); step_m++) {
        ...
        // bunch injection
        // TODO: Where is correct location for this piece of code? Beginning/end of step? Before
        // field solve?
        ...
        if (itsBunch_m->hasFieldSolver()) {
            ...
            // if field solver is not available , only update bunch, to transfer particles between
            // nodes if needed, reset parameters such as LocalNum, initialTotalNum_m. *ippl::Info <<
            // "No space charge Effects are included"<<endl;
            if ((step_m % Options::repartFreq * 100) == 0) {  // TODO: why * 100?
                Vector_t<double, 3> const meanP = calcMeanP();
                double const phi = calculateAngle(meanP(0), meanP(1)) - 0.5 * Physics::pi;
                Vector_t<double, 3> const meanR = calcMeanR();
        ...
        // recalculate bingamma and reset the BinID for each particles according to its current
        // gamma
        ...
        if (mode_m == TrackingMode::SINGLE) {
            ...
            double temp_meanTheta = calculateAngle2(
                itsBunch_m->R[i](0),
                itsBunch_m->R[i](1));  // [-pi, pi]

            dumpThetaEachTurn_m(
                t, itsBunch_m->R[i], itsBunch_m->P[i], temp_meanTheta, finishedTurn);

            dumpAzimuthAngles_m(
                t, itsBunch_m->R[i], itsBunch_m->P[i], oldReferenceTheta, temp_meanTheta);
            ...
            if (isTurnDone()) {
                ...
                *gmsg << "*** Finished turn " << turnnumber_m - 1
                      << ", Total number of live particles: " << itsBunch_m->getTotalNum() << endl;
            }
...
void ParallelCyclotronTracker::GenericTracker() {
    ...
    // mbHandler_m->getNumBunch() determines the number of bunches in multibunch mode (MBM, 1 for
    // OFF) Total number of particles determines single particle mode (SPM, 1 particle) or Static
    // Equilibrium Orbit Mode (SEO, 2 particles)
    ...
    for (; (step_m < maxSteps_m) && (itsBunch_m->getTotalNum() > 0); step_m++) {
        ...
        switch (mode_m) {
            ...
            case TrackingMode::UNDEFINED: {
                default:
                    throw OpalException(
                        "ParallelCyclotronTracker::GenericTracker()", "No such tracking mode.");
            }
        }
...
bool ParallelCyclotronTracker::getFieldsAtPoint(
    const double& t, const size_t& Pindex, Vector_t<double, 3>& Efield,
    Vector_t<double, 3>& Bfield) {
...
bool ParallelCyclotronTracker::checkGapCross(
    Vector_t<double, 3> Rold, Vector_t<double, 3> Rnew, RFCavity* rfcavity, double& Dold) {
    bool flag   = false;
...
bool ParallelCyclotronTracker::RFkick(
    RFCavity* rfcavity, const double t, const double dt, const int Pindex) {
    double radius = std::sqrt(
        std::pow(itsBunch_m->R[Pindex](0), 2.0) + std::pow(itsBunch_m->R[Pindex](1), 2.0)
    ...
    if (nomalRadius <= 1.0 && nomalRadius >= 0.0) {
        ...
        rfcavity->getMomentaKick(
            nomalRadius, tempP, t, dt, itsBunch_m->ID[Pindex], itsBunch_m->getM(),
            itsBunch_m->getQ());  // t : ns
 ...
 * @return
 */
bool ParallelCyclotronTracker::getTunes(
    dvector_t& t, dvector_t& r, dvector_t& z, int lastTurn, double& /*nur*/, double& /*nuz*/) {
    TUNE_class* tune;
    ...
    *gmsg << "* ************************************* nuR "
          ...
          << endl;
    *gmsg << endl
          << "* ===> " << Ndat << " data points  Ti=" << ti << " Tf= " << tf << " -> T= " << T
          << endl;
...
Vector_t<double, 3> ParallelCyclotronTracker::calcMeanR(short bunchNr) const {
    Vector_t<double, 3> meanR(0.0, 0.0, 0.0);
    ...
    return meanR / Vector_t<double, 3>(n);
}

Vector_t<double, 3> ParallelCyclotronTracker::calcMeanP() const {
    Vector_t<double, 3> meanP(0.0, 0.0, 0.0);
    ...
    return meanP / Vector_t<double, 3>(itsBunch_m->getTotalNum());
}
...
void ParallelCyclotronTracker::globalToLocal(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, double phi,
    Vector_t<double, 3> const translationToGlobal) {
...
void ParallelCyclotronTracker::localToGlobal(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, double phi,
    Vector_t<double, 3> const translationToGlobal) {
    IpplTimings::startTimer(TransformTimer_m);
...
inline void ParallelCyclotronTracker::globalToLocal(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, Quaternion_t const quaternion,
    Vector_t<double, 3> const meanR) {
    IpplTimings::startTimer(TransformTimer_m);
...
inline void ParallelCyclotronTracker::localToGlobal(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, Quaternion_t const quaternion,
    Vector_t<double, 3> const meanR) {
    IpplTimings::startTimer(TransformTimer_m);
...
inline void ParallelCyclotronTracker::globalToLocal(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, double const phi, double const psi,
    Vector_t<double, 3> const meanR) {
    IpplTimings::startTimer(TransformTimer_m);
...
inline void ParallelCyclotronTracker::globalToLocal(
    Vector_t<double, 3>& myVector, double const phi, double const psi,
    Vector_t<double, 3> const meanR) {
    IpplTimings::startTimer(TransformTimer_m);
    // double const tolerance = 1.0e-4; // TODO What is a good angle threshold? -DW
...
inline void ParallelCyclotronTracker::rotateWithQuaternion(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, Quaternion_t const quaternion) {
    Vector_t<double, 3> const quaternionVectorComponent =
        Vector_t<double, 3>(quaternion(1), quaternion(2), quaternion(3));
    double const quaternionScalarComponent = quaternion(0);

    for (unsigned int i = 0; i < itsBunch_m->getLocalNum(); ++i) {
        particleVectors[i] =
            2.0f * dot(quaternionVectorComponent, particleVectors[i]) * quaternionVectorComponent
            + (quaternionScalarComponent * quaternionScalarComponent
               - dot(quaternionVectorComponent, quaternionVectorComponent))
                  * particleVectors[i]
            + 2.0f * quaternionScalarComponent
...
inline void ParallelCyclotronTracker::normalizeVector(Vector_t<double, 3>& vector) {
    double tolerance = 1.0e-10;
...
inline void ParallelCyclotronTracker::rotateAroundZ(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, double const phi) {
...
inline void ParallelCyclotronTracker::rotateAroundZ(
    Vector_t<double, 3>& myVector, double const phi) {
...
inline void ParallelCyclotronTracker::rotateAroundX(
    ParticleAttrib<Vector_t<double, 3>>& particleVectors, double const psi) {
...
inline void ParallelCyclotronTracker::rotateAroundX(
    Vector_t<double, 3>& myVector, double const psi) {
...
inline void ParallelCyclotronTracker::getQuaternionTwoVectors(
    Vector_t<double, 3> u, Vector_t<double, 3> v, Quaternion_t& quaternion) {
    ...
    Vector_t<double, 3> resultVectorComponent;
...
bool ParallelCyclotronTracker::push(double h) {
    ...
    for (unsigned int i = 0; i < itsBunch_m->getLocalNum(); ++i) {
        Vector_t<double, 3> const oldR = itsBunch_m->R[i];
        double const gamma             = std::sqrt(1.0 + dot(itsBunch_m->P[i], itsBunch_m->P[i]));
        double const c_gamma           = Physics::c / gamma;
        Vector_t<double, 3> const v    = itsBunch_m->P[i] * c_gamma;
        itsBunch_m->R[i] += h * v;
        for (const auto& ccd : cavCrossDatas_m) {
            double const distNew =
                (itsBunch_m->R[i][0] * ccd.sinAzimuth - itsBunch_m->R[i][1] * ccd.cosAzimuth)
                - ccd.perpenDistance;
            bool tagCrossing = false;
            double distOld;
            if (distNew <= 0.0) {
                distOld =
                    (oldR[0] * ccd.sinAzimuth - oldR[1] * ccd.cosAzimuth) - ccd.perpenDistance;
                if (distOld > 0.0)
                    tagCrossing = true;
            }
            if (tagCrossing) {
...
bool ParallelCyclotronTracker::kick(double h) {
    ...
    double const M =
        itsBunch_m->M[0] * Units::GeV2eV;  // For now all particles have the same rest energy
...
void ParallelCyclotronTracker::borisExternalFields(double h) {
    ...
    IpplTimings::startTimer(IntegrationTimer_m);
    for (unsigned int i = 0; i < itsBunch_m->getLocalNum(); ++i) {
        itsBunch_m->Ef[i] = Vector_t<double, 3>(0.0, 0.0, 0.0);
        itsBunch_m->Bf[i] = Vector_t<double, 3>(0.0, 0.0, 0.0);

        computeExternalFields_m(
            i, itsBunch_m->getT() * Units::s2ns, itsBunch_m->Ef[i], itsBunch_m->Bf[i]);
    }
    IpplTimings::stopTimer(IntegrationTimer_m);
    ...
    // destroy particles if they are marked as Bin=-1 in the plugin elements or out of global
    // apeture
    deleteParticle(flagNeedUpdate);
}
...
bool ParallelCyclotronTracker::deleteParticle(bool flagNeedUpdate) {
    ...
    if (flagNeedUpdate) {
        ...
        size_t sum = std::accumulate(totalnum.begin(), totalnum.end() - 1, 0);
        ...
        size_t locNumLost =
            std::accumulate(locLostParticleNum.begin(), locLostParticleNum.end(), 0);

        reduce(locNumLost, globLostParticleNum, 1, std::plus<size_t>());

        if (globLostParticleNum > 0) {
            *gmsg << level3 << "At step " << step_m << ", lost " << globLostParticleNum
                  << " particles" << endl;
        }
        ...
        globalToLocal(
            ...
            Vector_t<double, 3>(0.0));  // P should be rotated, but not shifted

        // itsBunch_m->R *= Vector_t<double, 3>(0.001); // mm --> m
        ...
        // itsBunch_m->R *= Vector_t<double, 3>(1000.0); // m --> mm

        localToGlobal(itsBunch_m->R, phi, psi, meanR);
        localToGlobal(itsBunch_m->P, phi, psi, Vector_t<double, 3>(0.0));
...
void ParallelCyclotronTracker::initTrackOrbitFile() {
    ...
    if (myNode_m == 0) {
        if (OpalData::getInstance()->inRestartRun()) {
            ...
            outfTrackOrbit_m << "# Restart at integration step " << itsBunch_m->getLocalTrackStep()
                             << std::endl;
        } else {
            ...
            outfTrackOrbit_m
                ...
                << std::endl;
            outfTrackOrbit_m << "# Part. ID    x [m]       beta_x*gamma       y [m]      "
                                "beta_y*gamma        z [m]      beta_z*gamma"
                             << std::endl;
        }
    }
}
...
void ParallelCyclotronTracker::initDistInGlobalFrame() {
    if (!OpalData::getInstance()->inRestartRun()) {
        ...
        if (initialTotalNum_m > 2) {
            for (size_t i = 0; i < initialLocalNum_m; ++i) {
                if (itsBunch_m->ID[i] == 0) {
                    itsBunch_m->R[i] = Vector_t<double, 3>(0.0);
                    itsBunch_m->P[i] = Vector_t<double, 3>(0.0);
                }
            }
        }
        ...
        Vector_t<double, 3> const initMeanR = Vector_t<double, 3>(
            Units::mm2m * referenceR * cosRefTheta_m, Units::mm2m * referenceR * sinRefTheta_m,
...
void ParallelCyclotronTracker::checkFileMomentum() {
    ...
    if (tolerance > 0 && std::abs(pTotalMean - referencePtot) / pTotalMean > tolerance) {
    ...
    if (ippl::Comm->size() > 1) {
        ...
        if (myNode_m == 0) {
            ...
            while (notReceived > 0) {
                ...
                if (rmsg == nullptr)
                    *ippl::Error << "Could not receive from client nodes in main." << endl;
            ...
            if (!Ippl::Comm->send(smsg, 0, tag)) {
                *ippl::Error << "Ippl::Comm->send(smsg, 0, tag) failed " << endl;
            }
        }
...
void ParallelCyclotronTracker::bunchDumpStatData() {
    ...
    if (isMultiBunch()) {
        ...
        Vector_t<double, 3> meanR = calcMeanR();
        ...
        if (Options::psDumpFrame != DumpFrame::GLOBAL) {
            Vector_t<double, 3> meanP = calcMeanP();
            ...
            // Rotate so Pmean is in positive y direction. No shift, so that normalized emittance
            // and unnormalized emittance as well as centroids are calculated correctly in
    ...
    Vector_t<double, 3> meanR;
    Vector_t<double, 3> meanP;
...
void ParallelCyclotronTracker::bunchDumpPhaseSpaceData() {
    ...
    referencePt   = std::sqrt(
        referencePtot * referencePtot - referencePz * referencePz - referencePr * referencePr);
    ...
    if (Options::psDumpFreq < std::numeric_limits<int>::max()) {
        ...
        if (dumpLocalFrame == true) {
            ...
            // The bunch is transformed into a local coordinate system with meanP in direction of
            // y-axis
            globalToLocal(itsBunch_m->R, phi, psi, meanR);
            // globalToLocal(itsBunch_m->R, phi, psi, meanR * Vector_t<double, 3>(0.001));
        ...
        lastDumpedStep_m = itsDataSink->dumpH5(
            itsBunch_m,  // Local and in m
            FDext_m, E, referencePr, referencePt, referencePz, referenceR, referenceTheta,
            referenceZ,
        ...
        if (dumpLocalFrame == true) {
            ...
            localToGlobal(itsBunch_m->R, phi, psi, meanR);
            // localToGlobal(itsBunch_m->R, phi, psi, meanR * Vector_t<double, 3>(0.001));
            localToGlobal(itsBunch_m->P, phi, psi);
        }
        ...
        if (lastDumpedStep_m == -1) {
            *gmsg << endl
                  << "* Integration step " << step_m + 1
                  << " (no phase space dump for <= 2 particles)" << endl;
        } else {
            *gmsg << endl
                  << "* Phase space dump " << lastDumpedStep_m << " (" << dumpString
                  << " frame) at integration step " << step_m + 1 << endl;
        }
    }
...
void ParallelCyclotronTracker::update_m(double& t, const double& dt, const bool& finishedTurn) {
    ...
    if (itsBunch_m->getTotalNum() > 0) {
        ...
        if (mode_m != TrackingMode::SEO
            && (((step_m + 1) % Options::psDumpFreq == 0)
        ...
        if (mode_m != TrackingMode::SEO
            && (((step_m + 1) % Options::statDumpFreq == 0)
                || (Options::psDumpEachTurn && finishedTurn))) {
...
std::tuple<double, double, double> ParallelCyclotronTracker::initializeTracking_m() {
    ...
    setup_m.deltaTheta =
        Physics::pi / (setup_m.stepsPerTurn);  // half of the average angle per step
    ...
    if (OpalData::getInstance()->inRestartRun()) {
        ...
        *gmsg << "* Restart at integration step " << restartStep0_m << " at turn "
              << turnnumber_m - 1 << endl;
    ...
    *gmsg << "* The time step is set to dt = " << dt << " [ns]" << endl;
    ...
    if (isMultiBunch()) {
        ...
        *gmsg << "* MBM: The particles energy bin reset frequency is set to " << Options::rebinFreq
              << endl;
    }

    *gmsg << "* Single particle trajectory dump frequency is set to " << Options::sptDumpFreq
          << endl;
    *gmsg << "* The frequency to solve space charge fields is set to " << setup_m.scSolveFreq
          << endl;
    *gmsg << "* The repartition frequency is set to " << Options::repartFreq << endl;
    ...
    switch (mode_m) {
        case TrackingMode::SEO: {
            *gmsg << endl;
            *gmsg << "* ------------------------- STATIC EQUILIBRIUM ORBIT MODE "
                     "----------------------------- *"
                  << endl
                  << "* Instruction: When the total particle number is equal to 2, SEO mode is "
                     "triggered      *"
                  << endl
                  << "* automatically. This mode does NOT include any RF cavities. The initial "
                     "distribution   *"
                  << endl
                  << "* file must be specified. In the file the first line is for reference "
                     "particle and the  *"
                  << endl
                  << "* second line is for off-center particle. The tune is calculated by FFT "
                     "routines based  *"
                  << endl
        ...
        case TrackingMode::SINGLE: {
            *gmsg << endl;
            *gmsg << "* ------------------------------ SINGLE PARTICLE MODE "
                     "--------------------------------- *"
                  << endl
                  << "* Instruction: When the total particle number is equal to 1, single particle "
                     "mode is    *"
                  << endl
                  << "* triggered automatically. The initial distribution file must be specified "
                     "which should *"
                  << endl
        ...
        default: {
            throw OpalException(
                "ParallelCyclotronTracker::initializeTracking_m()", "No such tracking mode.");
        }
    }
...
void ParallelCyclotronTracker::finalizeTracking_m(
    dvector_t& Ttime, dvector_t& Tdeltr, dvector_t& Tdeltz, ivector_t& TturnNumber) {
    for (size_t ii = 0; ii < (itsBunch_m->getLocalNum()); ii++) {
        if (itsBunch_m->ID[ii] == 0) {
            double FinalMomentum2 = std::pow(itsBunch_m->P[ii](0), 2.0)
                                    + std::pow(itsBunch_m->P[ii](1), 2.0)
                                    + std::pow(itsBunch_m->P[ii](2), 2.0);
            double FinalEnergy =
                (std::sqrt(1.0 + FinalMomentum2) - 1.0) * itsBunch_m->getM() * Units::eV2MeV;
            *gmsg << "* Final energy of reference particle = " << FinalEnergy << " [MeV]" << endl;
            *gmsg << "* Total phase space dump number(includes the initial distribution) = "
                  << lastDumpedStep_m + 1 << endl;
            *gmsg << "* One can restart simulation from the last dump step (--restart "
                  << lastDumpedStep_m << ")" << endl;
        }
    }
    ...
    switch (mode_m) {
        case TrackingMode::SEO: {
            ...
            *gmsg << endl;
            *gmsg << "* **************** The result for tune calculation (NO space charge) "
                     "******************* *"
                  << endl
        ...
        default: {
            ...
            if (!isMultiBunch()) {
                *gmsg << "*" << endl;
                *gmsg << "* Finished during turn " << turnnumber_m << " (" << turnnumber_m - 1
                      << " turns completed)" << endl;
...
void ParallelCyclotronTracker::seoMode_m(
    double& t, const double dt, bool& /*finishedTurn*/, dvector_t& Ttime, dvector_t& Tdeltr,
    ...
    for (size_t i = 0; i < (itsBunch_m->getLocalNum()); i++) {
        if ((step_m % Options::sptDumpFreq == 0)) {
            outfTrackOrbit_m << "ID" << (itsBunch_m->ID[i]);
            outfTrackOrbit_m << " " << itsBunch_m->R[i](0) << " " << itsBunch_m->P[i](0) << " "
                             << itsBunch_m->R[i](1) << " " << itsBunch_m->P[i](1) << " "
                             << itsBunch_m->R[i](2) << " " << itsBunch_m->P[i](2) << std::endl;
        }

        double OldTheta = calculateAngle(itsBunch_m->R[i](0), itsBunch_m->R[i](1));
        r_tuning[i] =
            itsBunch_m->R[i](0) * std::cos(OldTheta) + itsBunch_m->R[i](1) * std::sin(OldTheta);
...
void ParallelCyclotronTracker::singleMode_m(
    double& t, const double dt, bool& finishedTurn, double& oldReferenceTheta) {
    ...
    if (itsBunch_m->getLocalNum() == 0)
        return;  // might happen if particle is in collimator

    IpplTimings::startTimer(IntegrationTimer_m);

    unsigned int i = 0;  // we only have a single particle
    if (step_m % Options::sptDumpFreq == 0) {
        outfTrackOrbit_m << "ID" << itsBunch_m->ID[i] << " " << itsBunch_m->R[i](0) << " "
                         << itsBunch_m->P[i](0) << " " << itsBunch_m->R[i](1) << " "
                         << itsBunch_m->P[i](1) << " " << itsBunch_m->R[i](2) << " "
                         << itsBunch_m->P[i](2) << std::endl;
    }

    double temp_meanTheta = calculateAngle2(itsBunch_m->R[i](0),
                                            itsBunch_m->R[i](1));  // [-pi, pi]

    dumpThetaEachTurn_m(t, itsBunch_m->R[i], itsBunch_m->P[i], temp_meanTheta, finishedTurn);

    dumpAzimuthAngles_m(t, itsBunch_m->R[i], itsBunch_m->P[i], oldReferenceTheta, temp_meanTheta);

    oldReferenceTheta = temp_meanTheta;

    // used for gap crossing checking
    Vector_t<double, 3> Rold = itsBunch_m->R[i];  // [x,y,z]    (mm)
    Vector_t<double, 3> Pold = itsBunch_m->P[i];  // [px,py,pz] (beta*gamma)
...
void ParallelCyclotronTracker::bunchMode_m(double& t, const double dt, bool& finishedTurn) {
    ...
    if (itsBunch_m->hasFieldSolver()) {
        if (step_m % setup_m.scSolveFreq == 0) {
            ...
            // accordingly. For this we find the quaternion between the previous mean momentum
            // (PreviousMeanP) and the current mean momentum (meanP) and rotate the fields with this
            // quaternion.
...
void ParallelCyclotronTracker::gapCrossKick_m(
    size_t i, double t, double dt, const Vector_t<double, 3>& Rold,
    const Vector_t<double, 3>& Pold) {
    for (beamline_list::iterator sindex = ++(FieldDimensions.begin());
         sindex != FieldDimensions.end(); ++sindex) {
...
void ParallelCyclotronTracker::dumpAzimuthAngles_m(
    const double& t, const Vector_t<double, 3>& R, const Vector_t<double, 3>& P,
    const double& oldReferenceTheta, const double& temp_meanTheta) {
    for (unsigned int i = 0; i <= 2; i++) {
        if ((oldReferenceTheta < azimuth_angle_m[i] - setup_m.deltaTheta)
            && (temp_meanTheta >= azimuth_angle_m[i] - setup_m.deltaTheta)) {
            *(outfTheta_m[i]) << "#Turn number = " << turnnumber_m << ", Time = " << t << " [ns]"
                              << std::endl
                              << " " << std::hypot(R(0), R(1)) << " "
...
void ParallelCyclotronTracker::dumpThetaEachTurn_m(
    const double& t, const Vector_t<double, 3>& R, const Vector_t<double, 3>& P,
    const double& temp_meanTheta, bool& finishedTurn) {
    if (isTurnDone()) {
        ++turnnumber_m;
        finishedTurn = true;
        ...
        *(outfTheta_m[3]) << "#Turn number = " << turnnumber_m << ", Time = " << t << " [ns]"
                          << std::endl
...
void ParallelCyclotronTracker::computeSpaceChargeFields_m() {
    ...
    if (spiral_flag && itsBunch_m->getFieldSolverType() == FieldSolverType::SAAMG) {
        ...
        itsBunch_m->setGlobalMeanR(Vector_t<double, 3>(0.0, 0.0, 0.0));
        ...
        Vector_t<double, 3> const meanR = calcMeanR();  // (m)
...
bool ParallelCyclotronTracker::computeExternalFields_m(
    const size_t& i, const double& t, Vector_t<double, 3>& Efield, Vector_t<double, 3>& Bfield) {
...
bool ParallelCyclotronTracker::computeExternalFields_m(
    ...
    Vector_t<double, 3>& Efield, Vector_t<double, 3>& Bfield) {
    beamline_list::iterator sindex = FieldDimensions.begin();
    // Flag whether a particle is out of field
...
void ParallelCyclotronTracker::injectBunch(bool& flagTransition) {
    ...
    const short result = mbHandler_m->injectBunch(itsBunch_m, itsReference, flagTransition);
...
void ParallelCyclotronTracker::updateAzimuthAndRadius() {
    ...
    for (short b = 0; b < mbHandler_m->getNumBunch(); ++b) {
        Vector_t<double, 3> meanR            = calcMeanR(b);
